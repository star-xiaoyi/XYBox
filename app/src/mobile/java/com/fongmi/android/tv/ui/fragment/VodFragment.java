...
public class VodFragment extends BaseFragment implements SiteCallback, FilterCallback, TypeAdapter.OnClickListener, ConfigCallback {
    ...
    public void setConfig(Config config) {
        Logger.d("VodFragment", "setConfig called with: " + (config != null ? config.toString() : "null"));
        
        if (config == null || config.isEmpty()) {
            Logger.d("VodFragment", "Config is null or empty, returning");
            return;
        }
        
        // 检查Fragment是否还在活动状态，增强检查
        if (!isValidFragmentState()) {
            Logger.d("VodFragment", "Fragment state invalid, returning");
            return;
        }
        
        Logger.d("VodFragment", "Fragment state valid, proceeding with config load");
        ...
        Logger.d("VodFragment", "Calling VodConfig.load");
        VodConfig.load(config, new Callback() {
            @Override
            public void success() {
                Logger.d("VodFragment", "VodConfig.load success callback");
                // 双重检查Fragment是否还在活动状态
                if (!isValidFragmentState()) {
                    Logger.d("VodFragment", "Fragment state invalid in success callback");
                    return;
                }
                
                try {
                    Logger.d("VodFragment", "Success: dismissing notify and refreshing");
                    Notify.dismiss();
                    RefreshEvent.config();
                    RefreshEvent.video();
                    homeContent();
                } catch (Exception e) {
                    Logger.e("VodFragment", "Error in success callback", e);
                    Logger.e("Error", e);
                }
            }
            ...
            public void error(String msg) {
                Logger.e("VodFragment", "VodConfig.load error: " + msg);
                // 双重检查Fragment是否还在活动状态
                if (!isValidFragmentState()) {
                    Logger.d("VodFragment", "Fragment state invalid in error callback");
                    return;
                }
                ...
                try {
                    ...
                    Logger.e("VodFragment", "Error in error callback", e);
                    Logger.e("Error", e);
                }
            }
        });
